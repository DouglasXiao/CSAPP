...
void doit(int connfd) {
	...
	char endserver_http_head[MAXLINE];
	...
	build_http_header(endserver_http_head, hostname, path, port, &rio);

	end_serverfd = connect_endServer(hostname, port, endserver_http_head);
	if (end_serverfd < 0) {
	...
	Rio_writen(end_serverfd, endserver_http_head, strlen(endserver_http_head));

	// receive massage from end server and send to client
...
void build_http_header(char *http_header, char *hostname, char *path, int port, rio_t *client_rio) {
	...
	sprintf(http_header, "%s%s%s%s%s%s%s", request_hdr, host_hdr, conn_hdr, prox_hdr, user_agent_hdr,
		other_hdr, endof_hdr);
