...
int fitsBits(int x, int n) {
  int shift = 33 + ~n;
  return !(x ^ ((x << shift) >> shift));  // Modified the test.c file
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  // get y's sign
  int sy = (y >> 31) & 1;
  // y - x and don't care about overflow
  int y_x = y + ~x + 1;
  int syx = (y_x >> 31) & 1;
  // two conditions:
  // 1. x is negative and y is positive
  // 2. the same sign between x and y, and y_x sign is 0
  return (sx & (!sy)) | (!(sx ^ sy) & !syx);
}
/*
...
unsigned float_neg(unsigned uf) {
 unsigned num = 0x80000000 ^ uf; // modify sign
 unsigned exp = 0xff << 23;
 // if NaN, just return itself
 if ((exp & uf) == exp && (((1 << 23) + (~0)) & uf)) {
  return uf;
 }
 return num;
}
